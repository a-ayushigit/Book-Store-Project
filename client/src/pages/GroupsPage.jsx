          ...
          <div key={i} className="flex flex-row sm:col-span-6 col-span-12 hover:cursor-pointer">
            <div className="flex flex-col h-full w-full dark:bg-yellow-700 bg-indigo-200 p-4 border-blue-500 border shadow-sm shadow-pink-800 ">
              ...
              <div className="flex flex-row gap-5 p-1 justify-between items-center ">
                <button onClick={() => { sendRequest(group._id) }} className={`${user._id === group.createdBy.id || group.members.some((member) => member.id === (user._id)) || group.moderators.some((moderator) => moderator.id === (user._id)) || group.isPublic === true ? "hidden" : "flex items-center justify-center m-1 bg-blue-500 text-white font-light rounded-xl p-3"}`}> Request to Join </button>
                <Link to={`/groups/${group._id}`} className={` ${group.isPrivate === true && ((group.createdBy.id !== user._id) && !(group.members.some((member) => member.id === (user._id))) && !(group.moderators.some((moderator) => moderator.id === (user._id)))) ? "hidden" : "flex underline font-semibold"}`}> View Group  </Link>
              </div>
            </div>
          </div>
        ))}
