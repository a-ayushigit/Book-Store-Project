...
const Modal = ({modal}  ,{handlePayment}) => {
    console.log(modal);
  return (
    <div className={`${(modal === true)?' border border-gray-600 sm:h-auto min-h-[15rem]  w-auto min-w-[25rem] sm:max-h-[500vh] sm:w-[35rem] flex self-center absolute top-14 flex-shrink transition-shadow duration-500 backdrop-blur-sm bg-white/30 flex-col z-50 overscroll-contain text-xs':'hidden'} `}>
      <p className="flex  font-bold px-2">Add your shipping details ....</p>
      ...
      <button onClick={handlePayment}  className="bg-green-900 w-40 h-10 rounded-full text-nowrap m-2 font-bold text-white">Confirm Shipping Address </button>
      </div>
